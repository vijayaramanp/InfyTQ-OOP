...
class Patient:
    ...
    def calculate_lab_test_charge(self):
        ...
        for test_id in self.__list_of_lab_test_ids:
            tmp = LabTestRepository.get_test_charge(test_id)
            if tmp !=- 1:
                self.__lab_test_charge += tmp
...
class LabTestRepository:
    ...
    def get_test_charge(lab_test_id):
        if lab_test_id in LabTestRepository.__list_of_hospital_lab_test_ids:
            indx=LabTestRepository.__list_of_hospital_lab_test_ids.index(lab_test_id)
            return LabTestRepository.__list_of_lab_test_charge[indx]
        else:
            return -1
